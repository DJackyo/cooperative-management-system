...
const AsistenciaAsambleaList = () => {
  ...
  const attendedRecords = useMemo(
    () => records.filter((record) => (record.asistio || "").toUpperCase() === "SI"),
    [records],
  );
    ...
    <Box sx={{ mt: 3 }}>
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, sm: 4 }}>
          <DashboardCard title="">
            <Typography variant="h4" fontWeight={800} lineHeight={1}>{records.length}</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Registros de asistencia</Typography>
          </DashboardCard>
        </Grid>
        <Grid size={{ xs: 12, sm: 4 }}>
          <DashboardCard title="">
            <Typography variant="h4" fontWeight={800} lineHeight={1}>{attendedRecords.length}</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>Asistencias confirmadas</Typography>
          </DashboardCard>
        </Grid>
        <Grid size={{ xs: 12, sm: 4 }}>
          <DashboardCard title="">
            <Typography variant="h4" fontWeight={800} lineHeight={1}>{filteredRecords.length}</Typography>
          ...
          <DashboardCard title="Filtros" subtitle="Consulta las asistencias por asociado, estado y fecha.">
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 4 }}>
                <TextField fullWidth size="small" label="Buscar asociado o identificación" value={search} onChange={(event) => setSearch(event.target.value)} />
              </Grid>
              <Grid size={{ xs: 12, sm: 4, md: 2 }}>
                <FormControl fullWidth size="small">
                  <InputLabel id="attendance-filter-label">Asistencia</InputLabel>
                  <Select labelId="attendance-filter-label" value={attendanceFilter} label="Asistencia" onChange={(event) => setAttendanceFilter(event.target.value)}>
              ...
              <Grid size={{ xs: 12, sm: 4, md: 3 }}>
                <TextField fullWidth size="small" type="date" label="Desde" value={fromDate} onChange={(event) => setFromDate(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
              </Grid>
              <Grid size={{ xs: 12, sm: 4, md: 3 }}>
                <TextField fullWidth size="small" type="date" label="Hasta" value={toDate} onChange={(event) => setToDate(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
              </Grid>
            </Grid>
          </DashboardCard>
        </Grid>
          ...
          <DashboardCard title="Registro de asistencia a asamblea" subtitle={`${filteredRecords.length} resultado${filteredRecords.length === 1 ? "" : "s"} con los filtros actuales`}>
            <StyledTable
              ...
              renderCell={(column, record) => {
                const asociado = record.asociado;
                const nombre = [asociado?.nombre1, asociado?.nombre2, asociado?.apellido1, asociado?.apellido2].filter(Boolean).join(" ");
